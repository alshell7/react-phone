...
// Borrows the call stream: never requests or stops microphone tracks.
import { jsx as _jsx, jsxs as _jsxs } from "react/jsx-runtime";
export type LiveWaveformProps = Omit<HTMLAttributes<HTMLDivElement>, "onError"> & {
  active?: boolean;
  animate?: boolean;
  processing?: boolean;
  /** Borrowed stream; caller retains ownership. No capture occurs without it. */
...
export const LiveWaveform = ({
  active: requestedActive = false,
  animate = true,
  processing: requestedProcessing = false,
  stream,
  barWidth: requestedBarWidth = 3,
  barGap: requestedBarGap = 1,
  barRadius = 1.5,
  barColor,
  fadeEdges = true,
  fadeWidth = 24,
  ...
  style,
  ...props
}: LiveWaveformProps): ReactElement => {
  const reduced = !usePhoneMotion().enabled || !animate;
  const active = requestedActive && !reduced;
  const processing = requestedProcessing && !reduced;
  const barWidth = Math.max(1, Number.isFinite(requestedBarWidth) ? requestedBarWidth : 3);
  const barGap = Math.max(0, Number.isFinite(requestedBarGap) ? requestedBarGap : 1);
  ...
  // Attach to an existing stream. Disconnecting the visual cannot end a call.
  useEffect(() => {
    ...
    try {
      ...
      const validFft = Number.isInteger(Math.log2(fftSize)) && fftSize >= 32 && fftSize <= 32768;
      analyser.fftSize = validFft ? fftSize : 256;
      ...
      historyRef.current = [];
      removeResume = resumeOnInteraction(context);
    } catch (error) {
      onError?.(error instanceof Error ? error : new Error("Audio visualization unavailable"));
    }
    return () => {
      source?.disconnect();
      removeResume();
  ...
  }, [active, stream, fftSize, smoothingTimeConstant, onError]);
  ...
  return /*#__PURE__*/_jsxs("div", {
    ...
    role: "img",
    ...props,
    children: [!active && !processing && /*#__PURE__*/_jsx("div", {
      "aria-hidden": "true",
      style: {
        position: "absolute",
        top: "50%",
