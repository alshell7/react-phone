// Generated from src/components/elevenlabs/orb.tsx. Run npm run package:build; do not edit.
"use client";
...
export function Orb({
  ...
  const [palette, setPalette] = useState<[string, string]>(["#2e5945", "#b8daa2"]);
  useEffect(() => {
    const canvas = document.createElement("canvas");
    canvas.width = canvas.height = 1;
    const ctx = canvas.getContext("2d");
    if (!ctx) return;
    const accent = canvasRef.current ? getComputedStyle(canvasRef.current).color : "#2e5945";
    ...
    const light = colors ? rgb(colors[1]) : base.map(n => Math.round(n + (255 - n) * 0.65));
  ...
  useEffect(() => {
    ...
    void Promise.all([import("three"), import("./orb-texture.ts")]).then(([THREE, {
      orbTextureUrl
    }]) => {
      if (disposed) return;
      ...
      const texture = new THREE.TextureLoader().load(orbTextureUrl, () => {
        textureReady = true;
      ...
      const uniforms = {
        ...
        uOffsets: new THREE.Uniform(new Float32Array(Array.from({
          length: 7
        }, () => random() * Math.PI * 2))),
      ...
      const resize = () => {
        ...
        gl.setSize(Math.max(1, bounds.width), Math.max(1, bounds.height), false);
      };
      ...
      const draw = (now: number) => {
        ...
        if (!textureReady || document.hidden || !visible || now - last < 33) return;
        const delta = Math.min((now - last) / 1000, 0.05);
        last = now;
        const config = live.current;
        const rate = config.animate && !config.reduced ? Math.max(0, Math.min(2, config.speed)) : 0;
        uniforms.uInputVolume.value += (clamp(rate > 0 ? config.getInputVolume?.() ?? 0 : 0) - uniforms.uInputVolume.value) * 0.2;
        uniforms.uOutputVolume.value += (clamp(rate > 0 ? config.getOutputVolume?.() ?? 0 : 0) - uniforms.uOutputVolume.value) * 0.2;
        motion += (0.1 + (1 - Math.pow(uniforms.uOutputVolume.value - 1, 2)) * 0.9 - motion) * 0.12;
  ...
  return /*#__PURE__*/_jsxs("div", {
    ...
    children: [renderer !== "webgl" && /*#__PURE__*/_jsx("span", {
      "aria-hidden": "true",
      style: {
        position: "absolute",
        inset: "7%",
