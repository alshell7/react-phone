// Generated from src/core/media.ts. Run npm run package:build; do not edit.
import { PhoneTones, type PhoneToneOptions, type PhoneToneEvent } from "./tones.ts";
...
export function microphoneMessage(error: unknown): string {
  ...
  if (name === "NotFoundError") return "No microphone found. Connect a microphone, then try again.";
  if (name === "NotReadableError" || name === "AbortError") return "The microphone is busy or disconnected. Check your device and close other apps using it.";
  if (name === "OverconstrainedError") return "The selected microphone is unavailable. Choose the default microphone and try again.";
  if (name === "TimeoutError") return "Microphone permission is still pending. Respond to your browser’s prompt, then try again.";
...
export class PhoneAudio {
  ...
  async play(stream: MediaStream, volume: number, sinkId: string): Promise<void> {
    this.element ??= new Audio();
    this.element.volume = volume;
    this.element.srcObject = stream;
    if (sinkId && "setSinkId" in this.element) await this.element.setSinkId(sinkId);
  ...
  async setOutputDevice(id: string): Promise<void> {
    ...
    if (!("setSinkId" in this.element)) {
      if (id) throw new Error("Audio output selection is not supported by this browser.");
      return;
    }
