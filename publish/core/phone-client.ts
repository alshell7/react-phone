// Generated from src/core/phone-client.ts. Run npm run package:build; do not edit.
...
import type { Caller, CallInfo, ConnectionStatus, PhoneCallbacks, PhoneClientOptions, PhoneError, PhoneEvent, PhoneEventType, PhoneSnapshot, SipConfig } from "./types.ts";
...
export class PhoneClient {
  ...
  constructor(options: PhoneClientOptions = {}) {
    ...
    if (options.preview?.enabled) this.snapshot = {
      ...this.snapshot,
  ...
  startPreview(): void {
    if (!this.options.preview) throw new Error("Preview must be enabled at construction.");
  ...
  private emit(type: PhoneEventType, value?: string | boolean): void {
    ...
    if (type === "connection") invoke(() => this.callbacks.onConnectionChange?.(connection));
    if (type === "incoming" && call) invoke(() => this.callbacks.onIncomingCall?.(call));
    if (type === "answered" && call) invoke(() => this.callbacks.onAnswered?.(call));
    if (type === "disconnected" && call) invoke(() => this.callbacks.onDisconnected?.(call));
    if (type === "error" && error) invoke(() => this.callbacks.onError?.(error));
  }
  ...
  async connect(config: SipConfig): Promise<void> {
    ...
    if (this.hasCall()) throw this.fail("call", "End the current call before changing accounts.");
    try {
      validateConfig(config);
    } catch (e) {
      throw this.fail("configuration", (e as Error).message);
    }
    if (typeof window === "undefined" || !window.isSecureContext || !window.RTCPeerConnection) {
      throw this.fail("unsupported", "Open this phone in a WebRTC-capable browser over HTTPS or localhost.");
    }
    this.disconnect();
    this.audio.unlock();
    ...
    try {
      ...
      throw this.fail("connection", "The calling engine could not load. Check your connection and try again.");
    }
    if (generation !== this.generation) return;
    let ua: UA;
    try {
      ...
      this.config = null;
      this.connection("error");
      throw this.fail("configuration", "Check the SIP address, authentication username, and WebSocket server.");
    }
    this.ua = ua;
    await new Promise<void>((resolve, reject) => {
      ...
      const stopWithError = (code: "connection" | "registration", message: string) => {
        if (!isCurrent()) return;
      ...
      const deadline = () => {
        ...
        this.registrationTimer = setTimeout(() => stopWithError("connection", "The SIP server did not respond. Check the server address, network, and TLS certificate, then reconnect."), config.registrationTimeoutMs ?? 20000);
      };
      deadline();
      ua.on("connecting", ({
        attempts
        ...
        if (!isCurrent()) return;
        if (attempts > (config.maxReconnectAttempts ?? 5)) stopWithError("connection", "Could not reconnect to the SIP server. Check your network and reconnect.");
      });
      ua.on("registered", () => {
        if (!isCurrent()) return;
        clearTimeout(this.registrationTimer);
      ...
      ua.on("registrationFailed", ({
        ...
        stopWithError("registration", auth ? "Registration was rejected. Check the SIP identity, authentication username, and password." : "Registration failed. Verify your account and registrar settings, then reconnect.");
      });
      ua.on("unregistered", () => {
        if (!isCurrent()) return;
        ...
        try {
          ...
          stopWithError("registration", "Your registration expired. Reconnect your SIP account.");
        }
      });
      ua.on("disconnected", () => {
        if (!isCurrent()) return;
      ...
      ua.on("newRTCSession", ({
        ...
        if (this.session || session.direction === "incoming" && this.hasCall()) {
          session.terminate({
            status_code: 486,
      ...
      try {
        ...
        stopWithError("connection", "Could not open the SIP connection. Verify the server address and reconnect.");
      }
    });
  }
  ...
  private async acquireMicrophone(token: number): Promise<MediaStream> {
    if (typeof navigator === "undefined" || !navigator.mediaDevices?.getUserMedia || !window.isSecureContext) {
      throw this.fail("unsupported", "Microphone access needs HTTPS or localhost and a supported browser.");
    }
    this.update({
    ...
    const constraints: MediaStreamConstraints = {
      audio: {
        ...
        ...(this.snapshot.inputDeviceId ? {
          deviceId: {
            exact: this.snapshot.inputDeviceId
          }
        } : {})
      },
      video: false
    ...
    try {
      ...
      this.update({
        microphone: error instanceof Error && error.name === "NotAllowedError" ? "denied" : "unavailable"
      });
  ...
  private ownStream(stream: MediaStream): void {
    ...
    for (const track of stream.getAudioTracks()) {
      track.addEventListener("ended", () => {
        ...
        this.fail("microphone", "Your microphone disconnected. Check your device before calling again.");
  ...
  async call(target: string, caller?: Partial<Caller>): Promise<void> {
    if (this.hasCall()) throw this.fail("call", "A call is already in progress.");
    if (this.snapshot.connection !== "registered") throw this.fail("call", "Connect your SIP account before calling.");
    let uri: string;
    try {
      uri = normalizeTarget(target, this.config?.uri ?? "sip:preview@localhost");
    } catch (error) {
      throw this.fail("call", (error as Error).message);
    }
    this.update({
      error: null,
      call: this.newCall("outgoing", {
        ...caller,
        number: target
      }),
    ...
    if (this.options.preview) {
      ...
      this.previewTimer = setTimeout(() => this.accepted(), this.options.preview.answerDelayMs ?? 1600);
      return;
    }
    this.audio.unlock();
    try {
      ...
      if (!this.snapshot.error) throw this.fail("call", "The call could not start. Check the number and try again.");
      throw error;
    }
  }

  /** The only way to inject a simulated incoming call; requires explicit preview mode. */
  simulateIncoming(caller: Caller = {
    name: "Alex Morgan",
    number: "7003",
    tags: ["Preview call"]
  }): void {
    if (!this.options.preview) throw new Error("Simulation is only available in preview mode.");
    if (this.hasCall()) throw this.fail("call", "End the current preview call first.");
    this.update({
      status: "incoming",
    ...
    this.audio.unlock();
    this.update({
      playbackBlocked: !this.audio.ring()
    });
    this.emit("incoming");
  }
  async answer(): Promise<void> {
    ...
    try {
      const stream = await this.acquireMicrophone(token);
      if (token !== this.mediaGeneration || !this.session || this.session.isEnded()) {
        stopStream(stream);
        return;
      }
      this.ownStream(stream);
      ...
      if (!this.snapshot.error) throw this.fail("call", "The call could not be answered. Please try again.");
      throw error;
    }
  }
  hangup(): void {
    this.endSession(this.snapshot.status === "incoming" ? "Declined" : this.snapshot.call?.answeredAt ? "Call ended" : "Cancelled");
  }
  ...
  private finish(reason: string): void {
    ...
    const completed = call && !call.endedAt ? {
      ...call,
      endedAt: Date.now(),
      endReason: reason
    } : null;
    ...
    this.update({
      ...(completed ? {
        call: completed,
        status: "ended",
        history: [completed, ...this.snapshot.history].slice(0, this.options.historyLimit ?? 50)
      } : {}),
      localStream: null,
      remoteStream: null,
      muted: false,
  ...
  private bindSession(session: RTCSession): void {
    ...
    const bindPeer = (peer: RTCPeerConnection) => {
      const receive = (track: MediaStreamTrack) => {
        ...
        if (session.direction === "outgoing" && !this.snapshot.call?.answeredAt) this.audio.stopRinging();
        void this.playAudio();
      };
      peer.addEventListener("track", ({
        track
      }) => receive(track));
      peer.getReceivers().forEach(({
        track
      }) => {
      ...
      peer.addEventListener("iceconnectionstatechange", () => {
        ...
        if (peer.iceConnectionState === "failed") {
          this.fail("call", "The audio connection failed. Check your network and STUN/TURN settings.");
          this.endSession("Audio connection failed");
        } else if (peer.iceConnectionState === "disconnected") {
          this.iceTimer = setTimeout(() => {
            if (current()) {
              this.fail("call", "The audio connection was lost. Check your network and call again.");
    ...
    const ended = (cause: string) => {
      if (!current()) return;
      this.session = null;
      const known = new Set(["Busy", "Rejected", "Canceled", "No Answer", "Unavailable", "Not Found", "Connection Error", "Request Timeout", "Terminated", "BYE"]);
      const reason = known.has(cause) ? cause : "Call ended";
      this.finish(reason);
      if (!this.snapshot.call?.answeredAt && !["Canceled", "Rejected"].includes(cause)) this.fail("call", `Call ended: ${reason}. Check the destination and try again.`);
    };
    session.on("ended", ({
      cause
    }) => ended(cause));
  ...
  async playAudio(): Promise<void> {
    ...
    if (!stream) {
      if (this.snapshot.status === "incoming") {
        ...
        if (this.snapshot.status === "incoming" && this.snapshot.call?.id === callId) this.update({
          playbackBlocked: !this.audio.ring()
        });
      }
      return;
    ...
    try {
      await this.audio.play(stream, this.snapshot.volume, this.snapshot.outputDeviceId);
      if (this.snapshot.remoteStream === stream) this.update({
        playbackBlocked: false
      });
    } catch {
      if (this.snapshot.remoteStream === stream) this.update({
  ...
  setHeld(held: boolean): void {
    if (!["active", "held"].includes(this.snapshot.status) || this.snapshot.transferPending) return;
    if (this.options.preview) {
      this.update({
        status: held ? "held" : "active"
      });
      this.emit("held", held);
      return;
    }
    try {
      ...
      throw this.fail("call", "Hold is temporarily unavailable while the call negotiates. Try again shortly.");
    }
  }
  sendDTMF(tone: string): void {
    if (!/^[0-9*#A-D]$/.test(tone)) throw this.fail("call", "Use a single keypad digit, *, #, or A–D.");
    if (!["active", "held"].includes(this.snapshot.status)) return;
    try {
      this.session?.sendDTMF(tone, {
        transportType: (this.config?.dtmfTransport ?? "RFC2833") as DTMF_TRANSPORT
      });
      this.emit("dtmf", tone);
      this.playKeypadTone(tone);
    } catch {
      throw this.fail("call", "Could not send the keypad tone. Check your PBX’s DTMF transport setting.");
    }
  }
  ...
  transfer(target: string): void {
    ...
    const callId = this.snapshot.call?.id;
    const current = () => this.snapshot.call?.id === callId && this.snapshot.transferPending;
    const failed = () => {
      if (!current()) return;
      clearTimeout(this.transferTimer);
      this.update({
      ...
      this.fail("transfer", "Transfer failed or timed out. Your call is still connected; check the destination and try again.");
    };
    const succeeded = () => {
      if (!current()) return;
  ...
  setInputDevice(id: string): void {
    if (this.hasCall()) throw this.fail("audio", "End the current call before changing microphones.");
    this.update({
      inputDeviceId: id
    });
  }
  async setOutputDevice(id: string): Promise<void> {
    try {
      ...
      throw this.fail("audio", "Could not switch speakers. Select an available output in your browser or system settings.");
    }
  }
}
