// Generated from src/core/tones.ts. Run npm run package:build; do not edit.
/** Local audio cues are synthesized; no files, network requests or microphone needed. */
export type PhoneToneEvent = "incoming" | "outgoing" | "ringing" | "connected" | "disconnected" | "dtmf";
...
export const phoneTonePresets: Readonly<Record<PhoneTonePreset, Readonly<Record<Exclude<PhoneToneEvent, "dtmf">, TonePattern>>>> = {
  gentle: {
    incoming: {
      steps: [{
        frequencies: [523.25],
...
export function dtmfFrequencies(digit: string): readonly number[] {
  ...
  return row < 0 || digit.length !== 1 ? [] : [[697, 770, 852, 941][row], [1209, 1336, 1477, 1633][rows[row].indexOf(digit)]];
}
...
export class PhoneTones {
  ...
  unlock(): void {
    ...
    try {
      ...
      void ctx.resume().then(() => {
        if (this.context === ctx && this.loop && !this.timer) this.start(this.loop);
      }).catch(() => {});
    } catch {
  ...
  private allowed(event: PhoneToneEvent): boolean {
    return this.options.enabled !== false && this.options.events?.[event] !== false && (this.options.volume ?? 0.35) > 0;
  }
  private schedule(pattern: TonePattern): void {
    const ctx = this.context;
    if (!ctx || ctx.state !== "running") return;
    const volume = Math.min(1, Math.max(0, this.options.volume ?? 0.35));
    for (const step of pattern.steps) for (const hz of step.frequencies) {
      ...
      gain.gain.linearRampToValueAtTime(volume * 0.12 / step.frequencies.length, start + Math.min(0.015, duration / 4));
      gain.gain.setValueAtTime(volume * 0.12 / step.frequencies.length, start + duration * 0.75);
  ...
  start(event: PhoneToneEvent, digit?: string, repeat = true): boolean {
    this.stop();
    if (event === "dtmf" && this.options.haptics && typeof navigator !== "undefined") {
      try {
        navigator.vibrate?.(8);
      } catch {
    ...
    const pattern: TonePattern = event === "dtmf" ? {
      steps: [{
        frequencies: dtmfFrequencies(digit ?? ""),
        durationMs: 85
      }]
    ...
    const begin = () => {
      if (token !== this.generation || !this.context || this.context.state !== "running") return;
      this.schedule(pattern);
      if (repeat && pattern.repeatMs) this.timer = setInterval(() => this.schedule(pattern), pattern.repeatMs);
    };
    begin();
    // One-shot feedback can await an in-flight user-gesture resume. Stale cues are discarded.
    if ((!repeat || !pattern.repeatMs) && this.context?.state === "suspended") void this.context.resume().then(begin).catch(() => {});
