// Generated from src/react/use-call-activity.ts. Run npm run package:build; do not edit.
"use client";

import { useEffect, useState } from "react";
import { usePhone } from "./phone-provider.ts";
import { getCallVisualState, type CallVisualState } from "../core/visual-state.ts";
...
export function useCallActivity(bandCount = 12): CallActivity {
  ...
  useEffect(() => {
    ...
    try {
      if (!preview) {
        ...
        for (const [key, stream] of [["input", state.localStream], ["output", state.remoteStream]] as const) {
          if (!stream?.getAudioTracks().some(track => track.readyState === "live") || key === "input" && state.muted) continue;
          ...
          readers.push(() => {
            ...
            const level = Math.min(1, Math.sqrt(time.reduce((sum, n) => sum + ((n - 128) / 128) ** 2, 0) / time.length) * 6);
            // Frequency bins covering speech (100–4000 Hz), not bin indices as Hz.
            const lo = Math.floor(100 * analyser.fftSize / context!.sampleRate);
            const hi = Math.min(frequency.length, Math.ceil(4000 * analyser.fftSize / context!.sampleRate));
            const bands = Array.from({
              length: count
            }, (_, i) => {
              const start = lo + Math.floor((hi - lo) * i / count);
              const end = Math.max(start + 1, lo + Math.floor((hi - lo) * (i + 1) / count));
              let sum = 0;
  ...
  }, [active, state.localStream, state.remoteStream, state.muted, preview, count]);
  return {
    ...levels,
    active,
