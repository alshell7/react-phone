...
export function BarVisualizer({
  ...
  const sequence = useMemo(() => {
    ...
    if (["connecting", "initializing", "ringing"].includes(state))
      return Array.from({ length: count }, (_, i) => [i, count - 1 - i]);
  ...
  useEffect(() => {
    ...
    const interval =
      state === "listening" ? 500 : state === "thinking" ? 150 : 160;
  ...
  return (
    ...
    >
      {Array.from({ length: count }, (_, i) => {
        const value =
          moving && speech ? Math.max(0, Math.min(1, bands[i] ?? 0)) : 0;
        const heightPct = Math.min(
          maxHeight,
          Math.max(minHeight, value * 100 + 5),
        );
        ...
        return (
          <span
            ...
            style={{
              ...
              transition: moving
                ? `transform ${motion.duration}ms cubic-bezier(0.16,1,0.3,1), opacity ${motion.duration}ms ease`
                : "none",
            }}
          />
        );
