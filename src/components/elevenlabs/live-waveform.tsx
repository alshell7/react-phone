...
export const LiveWaveform = ({
  ...
  const barWidth = Math.max(
    1,
    Number.isFinite(requestedBarWidth) ? requestedBarWidth : 3,
  );
  const barGap = Math.max(
    0,
    Number.isFinite(requestedBarGap) ? requestedBarGap : 1,
  );
  ...
  useEffect(() => {
    ...
    try {
      ...
      const validFft =
        Number.isInteger(Math.log2(fftSize)) &&
        fftSize >= 32 &&
        fftSize <= 32768;
      analyser.fftSize = validFft ? fftSize : 256;
      analyser.smoothingTimeConstant = Math.max(
        0,
        Math.min(1, smoothingTimeConstant),
      );
      ...
      onError?.(
        error instanceof Error
          ? error
          : new Error("Audio visualization unavailable"),
      );
    }
    return () => {
      source?.disconnect();
