...
export function PhoneProvider({
  ...
  const [ownedClient] = useState(() => new PhoneClient({ preview }));
  ...
  useEffect(() => {
    if (autoConnect && config && !preview)
      void client.connect(config).catch(() => {});
    // A new config identity intentionally re-registers. Memoize it in your host app.
  }, [autoConnect, config, client, preview]);
  return (
    <PhoneContext.Provider value={{ state, client, preview: Boolean(preview) }}>
      {children}
