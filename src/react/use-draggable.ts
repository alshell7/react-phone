...
export function useDraggable(
  ...
  option: boolean | PhoneDragOptions = false,
): PhoneDragHandle {
  const config = typeof option === "boolean" ? { enabled: option } : option;
  const enabled = config.enabled !== false;
  const [local, setLocal] = useState<PhonePosition>(
    config.initialPosition ?? { x: 0, y: 0 },
  );
  const [dragging, setDragging] = useState(false);
  const position = config.position ?? local;
  const committed = useRef(position);
  const latest = useRef({ config, position });
  latest.current = { config, position };
  const start = useRef<{
    id: number;
    x: number;
    y: number;
  ...
  useLayoutEffect(() => {
    committed.current = enabled ? position : { x: 0, y: 0 };
  });
  const clamp = (next: PhonePosition): PhonePosition => {
    ...
    const bounds =
      latest.current.config.bounds === "parent"
  ...
  return {
    ...
    handleProps: {
      ...
      onKeyDown(event) {
        ...
        if (event.key === "Home") {
          event.preventDefault();
          move(config.initialPosition ?? { x: 0, y: 0 });
